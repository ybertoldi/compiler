...
static void node_arena_free(void) {
...
static void state_push(long v) {
  assert(stp < STACK_SIZE);
  state_stack[stp++] = v;
}
static long state_pop(void) {
  assert(stp > 0);
  return state_stack[--stp];
}
static long state_peek(void) {
  assert(stp > 0);
  return state_stack[stp - 1];
}
...
static void symbol_push_nonterm(Symbol s) {
  assert(syp < STACK_SIZE);
  symbols_stack[syp++] = s;
}
static Symbol symbol_pop(void) {
  assert(syp > 0);
  return symbols_stack[--syp];
}
static Symbol symbol_peek(void) {
  assert(syp > 0);
  return symbols_stack[syp - 1];
}
...
static void printstate(long t) {
  ...
  for (int i = 0; i < syp; i++) {
    printf("%s%s",
           (symbols_stack[i].symbol_type >= TKTYPE_NUM_TOKENS)
               ? var2str(symbols_stack[i].symbol_type)
...
// TODO: extract meaning from type
...
#define ident_stmt() for (int tmp = 0; tmp < stms_depth + 1; tmp++) printf("  ")
  ...
  case ASTYPE_DECLARATION: {
    printf("VARDEC(var_type=%s,varname=\"%s\"", primtypestr(n->var_type), n->varname);
    if (n->init_expr != NULL){
      printf(", init_expr=");
  ...
  case ASTYPE_FUNCALL: {
    printf("CALL(func_name=\"%s\",", n->funcall_name);
  ...
  case ASTYPE_FUNCDEC: {
    printf("DECL(func_name = \"%s\", ", n->funcdec_name);
    printf("ret_type = \"%s\"",primtypestr(n->ret_type->literal_type));
    if (n->params.count > 0) {
      printf(", params = [");
      for (int i = 0; i < n->params.count; ++i){
        if (i != 0) printf(", ");
        print_ast_aux(n->params.elems[i], stms_depth);
      }
      printf("]");
    }
    printf(")");
  ...
  case ASTYPE_FUNCDEF: {
    stms_depth += 1;
    printf("FUNC_DEF(\n"); ident(); printf("decl=");
    print_ast_aux(n->func_decl, stms_depth);

    printf(",\n"); ident(); printf("stmts = ");
    print_ast_aux(n->func_stmts, stms_depth+1);

    printf(")");
  } break;

  case ASTTYPE_LITERAL_TYPE: {
    printf("%s", primtypestr(n->literal_type));
  } break;
  

  case ASTYPE_FUNCARGS: {
    assert(0 && "Unreachable");
  } break;
  }
}

void print_ast(AstNode *n) { print_ast_aux(n, 0); }
...
void grammar_test(char *buf) {
  ...
  while (1) {
    ...
    if (parse_state == PARSING_TOKENS &&
        (gt_state = goto_table[state_peek()][tklist->type]) != 0) {
...
static inline void consume_symbol(long expected) {
  if (expected != symbol_pop().symbol_type)
...
static AstNode *consume_get_bnop(BinopType op, long expct1, long expct2,
                                 long expct3) {
  AstNode *lhs, *rhs, *ret;
...
static void reduce_ast(Production *p) {
  ...
  case VAR_CONST: {
    Symbol s;
    long type = symbol_peek().symbol_type;
    if (type == TKTYPE_INT) {
      s = consume_get_symbol(TKTYPE_INT);
      s.symbol_type = VAR_CONST;
      s.node->type = ASTYPE_CONST_INT;
      s.node->ival = strtol(s.node->lexeme, NULL, 10);

      symbol_push_nonterm(s);

    } else if (type == TKTYPE_HEXINT) {
      s = consume_get_symbol(TKTYPE_INT);
      s.symbol_type = VAR_CONST;

      s.node->type = ASTYPE_CONST_INT;
      s.node->ival = strtol(s.node->lexeme, NULL, 16);

      symbol_push_nonterm(s);
    } else if (type == TKTYPE_OCTALINT) {
      s = consume_get_symbol(TKTYPE_INT);
      s.symbol_type = VAR_CONST;

      s.node->type = ASTYPE_CONST_INT;
      s.node->ival = strtol(s.node->lexeme, NULL, 8);
      symbol_push_nonterm(s);

    } else if (type == TKTYPE_FLOAT) {
      TODO("TKTYPE_FLOAT");
    } else if (type == TKTYPE_STRLIT) {
      TODO("TKTYPE_STRLIT");
    } else if (type == TKTYPE_KW_FALSE || type == TKTYPE_KW_TRUE) {
      TODO("TKTYPE_BOOLEANS");
    } else {
      assert(0 && "unreachable value for type CONST");
    }
  ...
  case VAR_LOG_OR: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_OR, VAR_EXPR, TKTYPE_LOGIC_OR, VAR_EXPR1);
  ...
  case VAR_LOG_AND: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_AND, VAR_EXPR1, TKTYPE_LOGIC_AND, VAR_EXPR2);
  ...
  case VAR_CMP_EQ: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_EQ, VAR_EXPR2, TKTYPE_EQUALS, VAR_EXPR3);
  ...
  case VAR_CMP_NEQ: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_NEQ, VAR_EXPR2, TKTYPE_NEQUALS, VAR_EXPR3);
  ...
  case VAR_CMP_LT: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_LT, VAR_EXPR3, TKTYPE_LT, VAR_EXPR4);
  ...
  case VAR_CMP_LEQT: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_LEQT, VAR_EXPR3, TKTYPE_LEQT, VAR_EXPR4);
  ...
  case VAR_CMP_GT: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_GT, VAR_EXPR3, TKTYPE_GT, VAR_EXPR4);
  ...
  case VAR_CMP_GEQT: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_GEQT, VAR_EXPR3, TKTYPE_GEQT, VAR_EXPR4);
  ...
  case VAR_MULT: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_MULT, VAR_EXPR5, TKTYPE_MULT, VAR_EXPR6);
  ...
  case VAR_DIV: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_DIV, VAR_EXPR5, TKTYPE_DIV, VAR_EXPR6);
  ...
  case VAR_SUM: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_SUM, VAR_EXPR4, TKTYPE_SUM, VAR_EXPR5);
  ...
  case VAR_SUB: {
    AstNode *binop_node =
        consume_get_bnop(BNOP_SUB, VAR_EXPR4, TKTYPE_SUB, VAR_EXPR5);
  ...
  case VAR_DEC: {
    ...
    dec->varname = consume_get_symbol(TKTYPE_NAME).node->lexeme;
  ...
  case VAR_ASSIGN: {
    ...
    consume_symbol(TKTYPE_ATTRIBUTION);
    n->assign_tgt = consume_get_symbol(TKTYPE_NAME).node;
  ...
  case VAR_FUNC_CALL: {
    AstNode *n;
    consume_symbol(TKTYPE_CLPAREN);
    n = consume_get_symbol(VAR_FUNC_CALL_ARGS).node;
    consume_symbol(TKTYPE_OPPAREN);
    n->funcall_name = consume_get_symbol(TKTYPE_NAME).node->lexeme;
  ...
  case VAR_FUNC_CALL_ARGS: {
    if (p->num_elems == 1) { // <ARGS> ::= <EXPR>;
      ...
      s.node = n;
      s.symbol_type = VAR_FUNC_CALL_ARGS;
      symbol_push_nonterm(s);
    } else { // <ARGS> ::= <ARGS> TKTYPE_COMMA <EXPR>;
      AstNode *expr = consume_get_symbol(VAR_EXPR).node;
      consume_symbol(TKTYPE_COMMA);
      Symbol s = consume_get_symbol(VAR_FUNC_CALL_ARGS);
  ...
  case VAR_FUNC_DEF: {
    consume_symbol(TKTYPE_CLBRACKET);
    AstNode *stms = consume_get_symbol(VAR_STMTS).node;
    consume_symbol(TKTYPE_OPBRACKET);
    AstNode *func_dec = consume_get_symbol(VAR_FUNC_DEC).node;

    AstNode *func_def = node_alloc();
    func_def->type = ASTYPE_FUNCDEF;
    func_def->func_decl = func_dec;
    func_def->func_stmts = stms;
    
    Symbol s = {VAR_FUNC_DEF, func_def};
    symbol_push_nonterm(s);
  } break;
  ...
  case VAR_FUNC_DEC: {
    ...
    consume_symbol(TKTYPE_CLPAREN);
    if (symbol_peek().symbol_type == VAR_FUNC_DEC_ARGS){
      func_dec = consume_get_symbol(VAR_FUNC_DEC_ARGS).node;
    } else {
      func_dec = node_alloc();
      func_dec->type = ASTYPE_FUNCDEC;
    }
    consume_symbol(TKTYPE_OPPAREN);

    char *name = consume_get_symbol(TKTYPE_NAME).node->lexeme;
    AstNode *tipo = consume_get_symbol(VAR_TIPO).node;

    func_dec->funcdec_name = name;
    func_dec->ret_type = tipo;
    Symbol s = {VAR_FUNC_DEC, func_dec};
    symbol_push_nonterm(s);
  } break;

  case VAR_FUNC_DEC_ARGS: {
    AstNode *func;
    char *name = consume_get_symbol(TKTYPE_NAME).node->lexeme;    
    AstNode *n = consume_get_symbol(VAR_TIPO).node;
    n->type = ASTYPE_DECLARATION;
    n->var_type = n->literal_type; 
    n->varname = name;
    n->init_expr = NULL;
    if (symbol_peek().symbol_type == TKTYPE_COMMA){
      consume_symbol(TKTYPE_COMMA);
      func = consume_get_symbol(VAR_FUNC_DEC_ARGS).node;
      DA_APPEND(func->params, n);
    } else {
      func = node_alloc();
      func->type = ASTYPE_FUNCDEC;
      func->funcdec_name = NULL;
      func->ret_type = NULL;
      DA_APPEND(func->params, n);
    }

    Symbol s = {VAR_FUNC_DEC_ARGS, func};
    symbol_push_nonterm(s);
  } break;
...
void gen_code_from_ast(AstNode *node);
...
int main() {
  ...
  exit(0);
  printf("\n--------------------------------------------------\n");
  printf("Generated code from ast:\n\n");
  gen_code_from_ast(end);
...
void gen_code_from_ast(AstNode *node) {
  switch (node->type) {
  case ASTYPE_STMTS: {
    for (int i = 0; i < node->stmt_arr.count; i++)
      gen_code_from_ast(node->stmt_arr.elems[i]);
  } break;
  ...
  default:
    assert(0 && "gen_code_from_ast: expression type not supported");
  }
}


/* TODO: Chamada de funcao sem argumentos
 *
 *
 *
 */
